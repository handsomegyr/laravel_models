    ...
    public function finishPay($out_trade_no)
    {
        ...
        try {
            try {
                ...
                $buyerInfo['buyer_id'] = $buyerInfo['id'];
                ...
                switch ($process_task) {
                    case 'predeposit':
                        ...
                        $this->modelPointsUser->addOrReduce(POINTS_CATEGORY3, $buyerInfo['buyer_id'], $buyerInfo['buyer_name'], $buyerInfo['buyer_avatar'], $out_trade_no, $orderPayInfo['created_at'], $pay_amount, "预付款充值", "支付金额￥{$pay_amount_yuan}已充值到您的云购账户");
                        
                        // 增加支付日志记录
                        $this->modelPayLog->recordLog($buyerInfo['buyer_id'], \App\Payment\Models\Log::TYPE1, $pay_amount, '预付款充值', $orderPayInfo);
                        
                        // 更新支付订单的信息
                        $this->modelOrderPay->incSuccessAndFailureCount($orderPayInfo['id'], 1, 0);
                    ...
                    default:
                        ...
                        foreach ($orderGoodsList as $order_id => $goodsList) {
                            foreach ($goodsList as $goodsInfo) {
                                ...
                                for ($i = 0; $i < $goodsInfo['goods_num']; $i ++) {
                                    ...
                                    if ($pay_amount >= $goods_amount) {
                                        ...
                                        if (! empty($orderPayInfo['is_points_used'])) {
                                            try {
                                                // $pointInfo = $this->modelPointsUser->addOrReduce(POINTS_CATEGORY1, $buyer_id, $buyerInfo['buyer_name'], $buyerInfo['buyer_avatar'], $orderPayInfo['id'], $orderPayInfo['created_at'], - $integral, "支付", "云购商品编码(2542873)消耗{$integral}福分");
                                            } catch (\Exception $e) {
                                                ;
                                            }
                                        }
                                        ...
                                        if (! empty($orderPayInfo['is_pd_used'])) {
                                            if ($predepositInfo['current'] >= $goods_amount) {
                                                try {
                                                    $predepositInfo = $this->modelPointsUser->addOrReduce(POINTS_CATEGORY3, $orderPayInfo['buyer_id'], $buyerInfo['buyer_name'], $buyerInfo['buyer_avatar'], $orderPayInfo['id'], $orderPayInfo['created_at'], - $goods_amount, "支付", "云购商品");
                                                    $total_amount += $goods_amount; // 总花费金额
                                                    $isCanBuy = true;
                                                } catch (\Exception $e) {
                                                    ;
                                ...
                                // 更新成功购买的标志为成功
                                $this->modelOrderGoods->updateIsSuccess($goodsInfo['id'], true, $lottery_code, $failure_num);
                                
                                // 记录会员动态
                                $this->modelMemberNews->log($buyerInfo['buyer_id'], $buyerInfo['buyer_name'], $buyerInfo['buyer_avatar'], $buyerInfo['buyer_register_by'], \App\Member\Models\News::ACTION1, $goodsInfo['id'], $goodsInfo);
                                // 如果成功购买次数大于0
                                if ($lottery_code_num > 0) {
                                    // 增加会员的积分
                        ...
                        if ($total_amount - $pay_amount > 0) {
                            ...
                            // 退回支付金额至预付款中
                            $this->modelPointsUser->addOrReduce(POINTS_CATEGORY3, $orderPayInfo['buyer_id'], $buyerInfo['buyer_name'], $buyerInfo['buyer_avatar'], getNewId(), $orderPayInfo['created_at'], $failure_amount, "支付", "云购失败，支付金额￥{$failure_amount_yuan}已退回到您的云购账户");
                        }
                        
                        // 更新支付订单的信息
                        $this->modelOrderPay->incSuccessAndFailureCount($orderPayInfo['id'], $success_count, $failure_count);
                        ...
                        if (! empty($buyerInfo['inviter_id'])) {
                            ...
                            if (! empty($invitationInfo)) {
                                ...
                                $this->modelPointsUser->addOrReduce(POINTS_CATEGORY1, $invitationInfo['user_id'], $invitationInfo['user_name'], $invitationInfo['user_headimgurl'], $orderPayInfo['id'], null, $pointsRuleInfo['points'], $pointsRuleInfo['item_category'], $pointsRuleInfo['item']);
                                $pointsRuleInfo = $this->modelPointsRule->getInfoByCategoryAndCode(POINTS_CATEGORY2, 'invitation');
                                $this->modelPointsUser->addOrReduce(POINTS_CATEGORY2, $invitationInfo['user_id'], $invitationInfo['user_name'], $invitationInfo['user_headimgurl'], $orderPayInfo['id'], null, $pointsRuleInfo['points'], $pointsRuleInfo['item_category'], $pointsRuleInfo['item']);
                                
                                // 增加该邀请函的价值
                                $this->modelInvitation->incWorth($invitationInfo['id'], $worth, 1);
                                // 增加领取记录的价值
                                $this->modelInvitationGotDetail->incWorth($invitationInfo['id'], $orderPayInfo['buyer_id'], $worth, 1);
                            }
                        }
                        
                        // 更新订单统计的信息
                        $orderPayInfo = $this->modelOrderPay->getInfoById($orderPayInfo['id']);
