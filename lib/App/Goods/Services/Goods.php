    ...
    public function createNewPeriodGoods($goods_commonid)
    {
        ...
        try {
            ...
            try {
                ...
                // 更新当期次数
                $retUpdate = $this->modelGoodsCommon->updateCurrentPeriod($goodsCommonInfo['id'], $period, $newPeriodGoodsInfo['id']);
                
                // 检查是否已经生成
                $prizeInfo = $this->modelPrize->getInfoByCode($newPeriodGoodsInfo['id']);
                if (! empty($prizeInfo)) {
                    throw new \Exception('新一期商品对应的奖品已生成', - 9);
                }
                // 生成新一期的商品对应的奖品
                $prizeInfo = $this->modelPrize->create($newPeriodGoodsInfo['id'], $newPeriodGoodsInfo['name'], true, true, 0, true);
                if (empty($prizeInfo)) {
                    throw new \Exception('新一期商品对应的奖品生成失败', - 10);
                }
                
                // 更新云购奖品
                $this->modelGoods->updateLotteryPrizeId($newPeriodGoodsInfo['id'], $prizeInfo['id']);
                
                // 根据总云购人次生成云购码
                for ($i = 0; $i < $newPeriodGoodsInfo['total_person_time']; $i ++) {
                    // 生成新一期的商品对应的云购码
                    $newPrizeCodeInfo = $this->modelPrizeCode->create($prizeInfo['id'], $newPeriodGoodsInfo['lottery_code'] + $i);
                    if (empty($newPrizeCodeInfo)) {
                        throw new \Exception('云购码生成失败', - 11);
                    }
                }
                // 生成抽奖规则
                $lotterRuleInfo = $this->modelLotteryRule->create(YUNGOU_ACTIVITY_ID, $prizeInfo['id'], $newPeriodGoodsInfo['total_person_time'], 10000);
                ...
                $result = array(
                    ...
                    'new_period_goods_id' => $newPeriodGoodsInfo['id']
                );
    ...
    public function lottery($goods_id)
    {
        ...
        try {
            ...
            try {
                ...
                // 获取中奖的订单商品记录
                $orderGoodsInfo = $this->modelOrderGoods->getInfoByGoodsIdAndLotteryCode($goodsInfo['id'], $prize_code);
                $order_no = getNewId();
                ...
                // 更新该订单商品记录的订单信息
                $this->modelOrderGoods->updateOrderNoById($orderGoodsInfo['id'], $order_no, $order_state);
                ...
                $result = array(
                    'order_goods_id' => $orderGoodsInfo['id'],
